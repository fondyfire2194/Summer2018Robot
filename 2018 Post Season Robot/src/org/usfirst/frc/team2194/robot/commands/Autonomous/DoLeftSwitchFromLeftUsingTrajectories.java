 ...
 *
 */
public class DoLeftSwitchFromLeftUsingTrajectories extends CommandGroup {

	public DoLeftSwitchFromLeftUsingTrajectories() {
		// Add Commands here:
		...
		// away from switch
		addSequential(new RunReverseTrajectory("LSW_L1", DriveTrainCanBus.LSW_L));
		addSequential(new ElevatorMoveToHeight(CubeHandler.ELEVATOR_TRAVEL_POSITION_INCHES));
		// turn ready to reverse toward scale
		addSequential(new ResetEncoders());
		addSequential(new RunTrajectory("LSW_L2", DriveTrainCanBus.LSW_L));
